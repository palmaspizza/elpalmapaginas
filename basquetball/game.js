...
let missiles = [];
let remoteMissiles = new Map(); // Mapa para misiles de otros jugadores
let unsubscribeMissiles = null; // Listener de Firebase para misiles
let isDead = false;
let respawnTimer = 0;
...
// ============================================================
// 3.5 INYECCIÓN DE ESTILOS CSS PARA CROSSHAIR (para que funcione aunque no tengas styles.css actualizado)
(function injectStyles() {
    const style = document.createElement('style');
    style.textContent = `
        .crosshair.revenge-active .crosshair-dot {
            background: #ff0000;
            box-shadow: 0 0 12px #ff0000;
        }
        .crosshair.revenge-active .crosshair-line {
            background: rgba(255, 0, 0, 0.9);
            box-shadow: 0 0 8px #ff0000;
        }
        .crosshair.revenge-active .crosshair-outer {
            border: 2px solid #ff0000;
            width: 45px;
            height: 45px;
            animation: crosshairPulse 0.5s infinite;
        }
    `;
...
function backToRooms() {
    ...
    if (unsubscribeMissiles) { unsubscribeMissiles(); unsubscribeMissiles = null; } // Limpiar listener misiles
...
function applyBallFromFirebase(d) {
    ...
    // Guardar información de posesión en userData para sincronización visual continua
    ball.userData.possessorId = d.possessorId || null;
    ball.userData.inAir = d.inAir || false;

    // ── CASO: Pelota reseteada al centro (sin poseedor, sin vuelo) ──
    if (!d.inAir && !d.possessorId && d.authorityId === null) {
        ball.position.set(d.x, d.y, d.z);
        ballInAir = false;
    ...
    // Si alguien más tiene la pelota, forzar limpieza de mi posesión local
    if (!d.inAir && d.possessorId && d.possessorId !== myPlayerId) {
        ...
        ballInAir = false;
        // La posición se actualizará en updateBallPhysics cada frame
        return;
    }
    ...
    if (d.inAir) {
        ...
        ball.userData.possessorId = null; // Limpiar poseedor si la pelota vuela
        ...
        if (ball.userData) {
            ...
            // Si Firebase dice que no hay poseedor, limpiarlo localmente también
            if (!d.possessorId) ball.userData.possessorId = null;
        }
...
function initThree() {
    ...
    scene.add(camera); // CRÍTICO: permite que objetos añadidos a la cámara (pistola HUD) se rendericen
...
function listenRevengeMissiles() {
    if (!currentRoomId) return;
    const missilesRef = ref(database, `salas/${currentRoomId}/misiles`);
    
    unsubscribeMissiles = onValue(missilesRef, (snapshot) => {
        const data = snapshot.val() || {};
        
        // 1. Añadir misiles nuevos de otros jugadores
        Object.entries(data).forEach(([key, d]) => {
            if (d.ownerId === myPlayerId) return; // Ignorar los míos (ya los tengo localmente)
            if (remoteMissiles.has(key)) return;  // Ya procesado
            ...
            // Intentar cargar textura del misil
            const loader = new THREE.TextureLoader();
            loader.load('assets/images/misil.png', (tex) => {
                mat.map = tex; mat.needsUpdate = true;
            });
            
            const mesh = new THREE.Mesh(geo, mat);
            
            const startPos = new THREE.Vector3(d.startPos.x, d.startPos.y, d.startPos.z);
            const initialDir = new THREE.Vector3(d.initialDir.x, d.initialDir.y, d.initialDir.z).normalize();
            
            mesh.position.copy(startPos);
            mesh.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), initialDir);
            
            // Luz roja para visibilidad
            const mLight = new THREE.PointLight(0xff2200, 2, 4);
            mesh.add(mLight);
            scene.add(mesh);
            ...
            if (d.targetId === myPlayerId) {
                targetMesh = myPlayerMesh;
            } else if (d.targetId) {
                const p = players.get(d.targetId);
                if (p) targetMesh = p.mesh;
            }

            remoteMissiles.set(key, {
                mesh,
                target: targetMesh,
                timer: d.timer || 4.0,
                initialDir
        ...
        // 2. Eliminar misiles que ya no están en Firebase
        remoteMissiles.forEach((m, key) => {
            if (!data[key]) {
                scene.remove(m.mesh);
                remoteMissiles.delete(key);
            }
        });
    });
}
...
function initGameWithRoom(startX, startZ) {
    ...
    myPlayerMesh.userData.bodyTypeIndex = myBodyType; // Asegurar índice para cámara
    myPlayerMesh.userData.team = myPlayerTeam;
    myPlayerMesh.hasGun = false; 
    ...
    // Escuchar jugadores de AMBOS equipos, pelota y misiles
    listenRoomPlayers();
    listenForDeaths(); 
    listenRevengeMissiles(); // Nueva función de escucha para misiles
if (currentRoomData && currentRoomData.puntos_ganar) {
    ...
    document.getElementById('shot-clock-value').textContent = '10';
...
function updateCamera() {
    ...
    if (hasRevengeWeapon) {
        ...
        const bt = bodyTypes[myPlayerMesh.userData.bodyTypeIndex !== undefined ? myPlayerMesh.userData.bodyTypeIndex : 0];
        const eyeHeight = bt.scaleY * 1.1;
        camera.position.set(pos.x, pos.y + eyeHeight, pos.z);
        
        // Dirección de la mirada basada en ángulos del ratón (lookAt lejano para evitar gimbal)
        const lookDist = 100;
        const lookAtTarget = new THREE.Vector3(
            pos.x - Math.sin(cameraAngleX) * lookDist * Math.cos(cameraAngleY),
            pos.y + eyeHeight - Math.sin(cameraAngleY) * lookDist,
            pos.z - Math.cos(cameraAngleX) * lookDist * Math.cos(cameraAngleY)
        );
        ...
        // Ocultar cabeza y hélice para que no estorben la vista en 1ra persona
        myPlayerMesh.children.forEach(child => {
            // Ocultar cabeza (SphereGeometry radio ~0.7) y hélice (BoxGeometry con propeller)
            if (child === myPlayerMesh.userData.head) child.visible = false;
            if (child === myPlayerMesh.userData.propeller) child.visible = false;
        });
        if (myPlayerMesh.userData.head) myPlayerMesh.userData.head.visible = false;
        if (myPlayerMesh.userData.propeller) myPlayerMesh.userData.propeller.visible = false;
    } else {
        camera.position.set(
        ...
        // Restaurar visibilidad de cabeza y hélice
        if (myPlayerMesh.userData.head) myPlayerMesh.userData.head.visible = true;
        if (myPlayerMesh.userData.propeller) myPlayerMesh.userData.propeller.visible = true;
    }
}
...
function stealBall() {
    ...
    if (dist < 5.0) {
        ...
        ball.userData.possessorId = myPlayerId; // Asegurar possessorId local
        ball.position.set(currentPosition.x, currentPosition.y + 1.2, currentPosition.z);
        ballAuthority = myPlayerId;
        lastStealTime = now;
        updatePossessionUI();
...
function updateBallPhysics(deltaTime) {
    ...
    // --- SINCRONIZACIÓN VISUAL DE POSESIÓN (Para todos los jugadores) ---
    // Si la pelota no está en el aire y hay un poseedor (local o remoto)
    if (!ballInAir && ball.userData.possessorId) {
        let possessorMesh = null;
        if (ball.userData.possessorId === myPlayerId) {
            possessorMesh = myPlayerMesh;
        } else {
            const p = players.get(ball.userData.possessorId);
            if (p) possessorMesh = p.mesh;
        }

        if (possessorMesh) {
            // Pegar la pelota al poseedor en tiempo real en todas las pantallas
            ball.position.set(possessorMesh.position.x, possessorMesh.position.y + 1.2, possessorMesh.position.z);
            
            // Si el poseedor soy yo, asegurar estado local y autoridad
            if (ball.userData.possessorId === myPlayerId) {
                if (possession !== 'player') {
                    possession = 'player';
                    updatePossessionUI();
                ...
                // Si el poseedor es otro, forzar limpieza de mi posesión
                if (possession === 'player') {
                    possession = null;
                    updatePossessionUI();
                }
    ...
    // Solo la autoridad computa la física de movimiento (gravedad, rebotes, canastas)
...
if (teamScores.blue >= targetPointsToWin || teamScores.red >= targetPointsToWin) {
    ...
    return; // Salir de updateBallPhysics para no seguir procesando
            ...
            // === ENTREGAR PISTOLA AL EQUIPO CONTRARIO (Víctima) ===
            const victimTeam = scoringTeam === 'blue' ? 'red' : 'blue';
            lastScoringTeam = victimTeam; 
            
            giveGunToRandomPlayerOfTeam(victimTeam);
...
function resetBallAfterScore() {
    ballInAir = false;
    // Verificar si mi equipo fue el que recibió la canasta (víctima)
    ...
    if (ball.userData) {
        ball.userData.inAir = false;
        ball.userData.velocity = null;
        ball.userData.shooterTeam = null;
        ball.userData.possessorId = null;
    }
    possession = null;
    ballAuthority = null; // ← CRÍTICO: liberar autoridad para que todos reciban la posición
    shotClock = 10; // Reiniciar shot clock a 10 segundos
    const sc = document.getElementById('shot-clock-value');
    if (sc) sc.textContent = shotClock;
    ball.position.set(0, 0.8, 0);
    updatePossessionUI();
    // Forzar sincronización inmediata para que todos reciban la posición central
    const ballRef = ref(database, 'salas/' + currentRoomId + '/bola');
    set(ballRef, {
        x: 0, y: 0.8, z: 0,
        vx: 0, vy: 0, vz: 0,
        inAir: false,
...
function giveRevengeWeapon() {
    ...
    // Activar crosshair especial rojo
    const crosshairEl = document.querySelector('.crosshair');
    if (crosshairEl) crosshairEl.classList.add('revenge-active');
    ...
    // ── Crear pistola HUD con textura (PlaneGeometry para mostrar imagen 2D) ──
    const gunLoader = new THREE.TextureLoader();
    const gunMesh = new THREE.Group();

    // Plano de la pistola (imagen texturada)
    const planeMat = new THREE.MeshBasicMaterial({
        color: 0xffffff,
        transparent: true,
        opacity: 1.0,
        side: THREE.DoubleSide,
        depthTest: false // Siempre delante de otros objetos
    });
    const planeMesh = new THREE.Mesh(new THREE.PlaneGeometry(0.55, 0.35), planeMat);
    
    // Cargar textura; fallback a color sólido si falla
    gunLoader.load(
        'assets/images/pistola.png',
        (tex) => { planeMat.map = tex; planeMat.needsUpdate = true; },
        undefined,
        () => {
            // Fallback: rectángulo gris metálico visible
            planeMat.color.setHex(0x888888);
            planeMat.transparent = false;
        }
    );
    
    gunMesh.add(planeMesh);
    
    // Posicionamiento HUD: derecha, abajo, cerca de la cámara
    gunMesh.position.set(0.38, -0.28, -0.6);
    gunMesh.rotation.y = -0.15; // Ligera inclinación para aspecto natural
    gunMesh.renderOrder = 999; // Renderizar encima de todo
    
    // Añadir a la cámara (scene.add(camera) se llama en initThree para que funcione)
    camera.add(gunMesh);
    revengeWeaponMesh = gunMesh;

    // Flash UI
    const flash = document.createElement('div');
    flash.className = 'yellow-flash';
    flash.innerHTML = '<div class="dispara-text">APUNTA Y DISPARA!</div>';
    document.body.appendChild(flash);
    setTimeout(() => flash.remove(), 700);

    const timer = document.createElement('div');
    timer.id = 'revenge-timer';
    timer.style.cssText = 'position:fixed;top:60%;left:50%;transform:translate(-50%,-50%);color:white;font-size:36px;font-weight:bold;z-index:999;font-family:Orbitron;text-shadow:0 0 10px red;pointer-events:none;';
...
function discardWeapon() {
    ...
    // Desactivar crosshair especial
    const crosshairEl = document.querySelector('.crosshair');
    if (crosshairEl) crosshairEl.classList.remove('revenge-active');

    const timerEl = document.getElementById('revenge-timer');
    if (timerEl) timerEl.remove();

    if (revengeWeaponMesh) {
        camera.remove(revengeWeaponMesh);
        // Limpiar material para liberar memoria
        revengeWeaponMesh.traverse(child => {
            if (child.isMesh) {
    ...
    revengeDisappearTimer = 0;
}

function shootMissile() {
    if (!hasRevengeWeapon || isDead) return;
    
    // Desactivar crosshair y arma inmediatamente
    const crosshairEl = document.querySelector('.crosshair');
    if (crosshairEl) crosshairEl.classList.remove('revenge-active');
    ...
    // Detectar si apunta al rival con raycaster (crosshair = centro de pantalla)
    ...
    players.forEach((p, id) => {
        if (p.team !== myPlayerTeam && p.mesh) {
            p.mesh.userData.playerId = id;
            potentialTargets.push(p.mesh);
        }
    });
    ...
    if (intersects.length > 0) {
        ...
        // Subir al grupo padre que tiene el playerId
        while (obj && obj.parent && !obj.userData.playerId) obj = obj.parent;
        if (obj && obj.userData.playerId) {
            targetPlayerId = obj.userData.playerId;
            const tp = players.get(targetPlayerId);
            targetPlayerMesh = tp ? tp.mesh : null;
        }
    }
    ...
    // ── Crear misil local con textura (CylinderGeometry) ──
    const missileLoader = new THREE.TextureLoader();
    const missileGeo = new THREE.CylinderGeometry(0.12, 0.12, 0.9, 8);
    ...
    // Intentar cargar textura; si carga, aplicar sobre el color base
    missileLoader.load(
        'assets/images/misil.png',
        (tex) => { missileMat.map = tex; missileMat.needsUpdate = true; },
        undefined,
        () => { /* fallback: ya tiene color sólido rojo */ }
    );
    
    const missileMesh = new THREE.Mesh(missileGeo, missileMat);
    // Orientar el cilindro en la dirección del disparo
    missileMesh.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), initialDir);
    missileMesh.position.copy(startPos);
    
    // Luz roja en el misil para que se vea
    const missileLight = new THREE.PointLight(0xff2200, 2, 4);
    missileMesh.add(missileLight);
    
    scene.add(missileMesh);
    
    // Guardar en Firebase para que otros jugadores vean el misil
    const missileRef = push(ref(database, `salas/${currentRoomId}/misiles`));
    const missileData = {
        startPos: { x: startPos.x, y: startPos.y, z: startPos.z },
        initialDir: { x: initialDir.x, y: initialDir.y, z: initialDir.z },
        targetId: targetPlayerId,
        timer: 4.0,
        ownerId: myPlayerId
    };
    set(missileRef, missileData);
    ...
    missiles.push({
        ...
        targetId: targetPlayerId,
        timer: 4.0,
        initialDir: initialDir.clone(),
        firebaseKey: missileRef.key
    });

    // Descartar arma tras disparar
    hasRevengeWeapon = false;
    const timerEl = document.getElementById('revenge-timer');
    if (timerEl) timerEl.remove();
    if (revengeWeaponMesh) {
    ...
    showMessage('🚀 ¡MISIL DISPARADO! ' + (targetPlayerId ? '¡Teledirigido al rival!' : 'Sin objetivo...'));
}

function updateMissiles(deltaTime) {
    const missileSpeed = 28; // 28 uds/seg
    
    // ── 1. Misiles locales (yo disparé) ──
    for (let i = missiles.length - 1; i >= 0; i--) {
        const m = missiles[i];
        m.timer -= deltaTime;
        
        if (m.target) {
            // Homing dinámico: siempre hacia la posición actual del objetivo
            const targetPos = m.target.position.clone().add(new THREE.Vector3(0, 1.0, 0));
            const dir = new THREE.Vector3().subVectors(targetPos, m.mesh.position).normalize();
            m.mesh.position.add(dir.clone().multiplyScalar(missileSpeed * deltaTime));
            
            // Orientar el cilindro en la dirección de movimiento
            m.mesh.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), dir);

            // Detección de impacto
            if (m.mesh.position.distanceTo(targetPos) < 0.8) {
                handleImpact(m.target, m.targetId);
                createExplosion(m.mesh.position.clone());
                if (m.firebaseKey) remove(ref(database, `salas/${currentRoomId}/misiles/${m.firebaseKey}`));
                scene.remove(m.mesh);
                missiles.splice(i, 1);
                continue;
            }
        } else {
            // Sin objetivo: trayectoria errática pero hacia adelante
            const wobble = new THREE.Vector3(
                Math.sin(m.timer * 8) * 0.15,
                Math.cos(m.timer * 6) * 0.1,
                0
            ).applyQuaternion(m.mesh.quaternion);
            const forward = m.initialDir.clone().multiplyScalar(missileSpeed * deltaTime);
            m.mesh.position.add(forward).add(wobble);
        }

        // Eliminar si expiró o salió de la cancha
        if (m.timer <= 0 || Math.abs(m.mesh.position.x) > 30 || Math.abs(m.mesh.position.z) > 20) {
            if (m.firebaseKey) remove(ref(database, `salas/${currentRoomId}/misiles/${m.firebaseKey}`));
            scene.remove(m.mesh);
            missiles.splice(i, 1);
        }
    ...
    // ── 2. Misiles remotos (otros dispararon) ──
    remoteMissiles.forEach((m, key) => {
        m.timer -= deltaTime;
        ...
        if (m.target) {
            ...
            // Si el objetivo somos nosotros y hay impacto → morir
            if (m.target === myPlayerMesh && !isDead) {
                if (m.mesh.position.distanceTo(myPlayerMesh.position) < 1.2) {
                    // El killer name lo sabremos del nodo Firebase de muertes (listenForDeaths lo maneja)
                    createExplosion(m.mesh.position.clone());
                    scene.remove(m.mesh);
                    remoteMissiles.delete(key);
                    return;
...
function handleImpact(targetMesh, targetId) {
    // Crear explosión en el punto de impacto
    if (targetMesh) createExplosion(targetMesh.position.clone());
    
    // Resolver ID si no se pasó directamente
    if (!targetId) {
        players.forEach((p, id) => {
...
function onKilled(killerName) {
    if (isDead) return; // Evitar doble muerte
    isDead = true;
    respawnTimer = 2.5;
    
    // Eliminar arma si la tiene
    if (hasRevengeWeapon) discardWeapon();
    
    // Ocultar jugador
    if (myPlayerMesh) myPlayerMesh.visible = false;
    
    // Perder posesión de la pelota
    if (possession === 'player') {
        possession = null;
        ballInAir = false;
        if (ball.userData) { ball.userData.inAir = false; ball.userData.velocity = null; }
    ...
    // Mensaje de muerte en pantalla
    const existingMsg = document.getElementById('death-msg');
    ...
    msg.style.cssText = [
        ...
        'color:#ff1111', 'font-size:52px', 'font-weight:900',
        'z-index:9000', 'font-family:Orbitron,sans-serif',
        'text-align:center', 'text-shadow:0 0 30px #ff0000',
        'background:rgba(0,0,0,0.75)', 'padding:30px 50px',
        'border-radius:20px', 'border:3px solid #ff0000',
        'pointer-events:none'
    ].join(';');
    msg.innerHTML = (killerName || 'Alguien') + ' te mató XD 💀<br>' +
        '<span id="respawn-countdown" style="font-size:28px;color:white;display:block;margin-top:10px;">REVIVIENDO EN: 2.5s</span>';
...
function updateRespawn(deltaTime) {
    ...
    if (respawnTimer <= 0) {
        ...
        if (myPlayerMesh) {
            ...
            // Respawn en base del equipo correspondiente
        ...
        showMessage('🟢 ¡Reviviste weón! Corre pa\' la cancha!');
    }
}
...
function updateShotClock() {
    ...
    // Solo decrementar si tengo la pelota y no está en el aire
    if (possession === 'player' && !ballInAir && !isDead) {
        shotClock--;
        const sc = document.getElementById('shot-clock-value');
        if (sc) {
            sc.textContent = shotClock;
            // Parpadeo rojo urgente en los últimos 3 segundos
            sc.style.color = shotClock <= 3 ? '#ff2222' : '';
            sc.style.textShadow = shotClock <= 3 ? '0 0 10px red' : '';
        }
        
        if (shotClock <= 0) {
            shotClock = 10; // Reiniciar a 10
            if (sc) { sc.textContent = shotClock; sc.style.color = ''; sc.style.textShadow = ''; }
            
            // Perder la posesión (violación de shot clock)
            possession = null;
            ballInAir = true;
            ballAuthority = myPlayerId;
            
            // Lanzar la pelota al aire en dirección aleatoria
            const rx = (Math.random() - 0.5) * 10;
            const rz = (Math.random() - 0.5) * 10;
            ball.userData = { inAir: true, velocity: new THREE.Vector3(rx, 8, rz), shooterTeam: null };
            
            updatePossessionUI();
            syncBallToFirebase();
            showMessage('⏰ ¡SHOT CLOCK! Se te acabó el tiempo, culiado 😂');
        }
    } else if (!possession && !ballInAir) {
        // Pelota libre: resetear el clock a 10
        shotClock = 10;
        const sc = document.getElementById('shot-clock-value');
        if (sc) { sc.textContent = shotClock; sc.style.color = ''; sc.style.textShadow = ''; }
    }
